...
mega_spells <- function(data,
  ...
  data %>%
    ...
    dplyr::mutate(
      csum = c(0,cumsum(as.numeric(dplyr::lead({{start_date}})))[-dplyr::n()]),
      cmax = c(0,cummax(as.numeric({{end_date}}))[-dplyr::n()]),
      csum_date = as.Date(csum, origin = "1970-01-01"),
      cmax_date = as.Date(cmax, origin = "1970-01-01")
      ) %>%
    dplyr::mutate(
      indx = c(0,
               cumsum(
                 as.numeric(dplyr::lead({{start_date}})) >
                   cummax(as.numeric({{end_date}}))
                      )[-dplyr::n()])
      ) %>%
    dplyr::group_by(indx,.add=TRUE) %>%
    dplyr::mutate(
      megaspell_start_date=min({{start_date}},na.rm=TRUE),
